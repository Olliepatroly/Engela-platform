...
0. **~~Apply Phase 3 migrations~~ DONE (2026-07-06).** The Supabase MCP was re-authorised;
   migrations `0016`-`0019` are all applied live, types regenerated, untyped casts replaced, and
   the RLS suite extended and verified against the live project (33 tests green). Tickable actions
   and message Ollie are live, plus the new clinical flags (SBAR + voice notes), PAR-Q health
   profiles, and session notes — see the 2026-07-06 `CHANGELOG.md` entry. Remaining follow-ups:
   the console-side reply UI for the message thread, a console editor for `client_health_profiles.details`
   (the team currently has no UI to fill the clinical details that prefill SBAR Background), and
   clinical sign-off of the PAR-Q wording (`CLAUDE.md` §7 #8).
